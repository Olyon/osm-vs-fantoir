...
FROM (
      ...
      (SELECT DISTINCT code_insee
      from bano_adresses
      WHERE ST_Intersects(geometrie_3857,ST_Buffer(ST_TileEnvelope($tz, $tx, $ty),1000,2)) AND
            source = 'BAN'),
      fantoir
      as
      (SELECT fantoir
    ...
    FROM   (SELECT * FROM bano_adresses WHERE st_transform(geometrie,3857) && ST_TileEnvelope($tz, $tx, $ty)) as bano_a
    JOIN   fantoir
    USING  (fantoir)
    ...
    SELECT nom,
     ...
     FROM bano_a
     WHERE ST_Intersects(geom_hull, ST_TileEnvelope($tz, $tx, $ty))
) AS q
  ) TO STDOUT;
  "
}
...
pyramide 13 4174 4176 2922 2924
